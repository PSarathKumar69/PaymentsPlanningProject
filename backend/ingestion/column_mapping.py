...
def find_duplicate_unique_codes(ws, sheet_map, unique_code_col):
    ...
    EXACT MATCH ONLY (Sarath's explicit decision, 2026-08-13): duplicate
    Unique Codes are only ever flagged on a literal exact string match
    (after strip()) of this column's value — never a fuzzy/similarity
    score. A difflib-based find_fuzzy_duplicate_unique_codes() used to run
    alongside this and got removed entirely: on the real production sheet
    it produced ~5,471 near-useless warnings, because most vendor codes
    share a formulaic prefix (e.g. "ARSV00400", "ARSV00401", ...) that
    scores as "similar" under any generic string-similarity ratio even
    though the codes are completely distinct, real vendor identities. That
    made the fuzzy note channel pure noise Finance had no way to act on —
    worse than not flagging at all. This function's own exact-match rule is
    unaffected and remains the only Unique Code duplicate check.
