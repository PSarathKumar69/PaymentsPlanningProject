"""Model 3 router — priority-bucket assignment + plan generation."""
from typing import Any

from fastapi import APIRouter

from backend.db.session import SessionLocal
from backend.models.model3_priority_bucket.bucketer import assign_buckets, generate_plan
...
@router.get("/models/3/buckets", response_model=list[dict[str, Any]])
def get_model3_buckets():
    return assign_buckets(session=None, as_of=None).to_dict(orient="records")


@router.get("/models/3/plan-runs", response_model=PlanRunHistoryResponse)
def get_model3_plan_runs():
    """Every PlanRun for Model 3's family ("model3" + "model3_regeneration")
    ...
    try:
        return build_plan_run_history_response(session, 3)
    finally:
        session.close()


@router.post("/models/3/generate-plan", response_model=GeneratePlanResponse)
def post_model3_generate_plan(body: GeneratePlanRequest):
    result = generate_plan(body.available_funds, session=None, as_of=None, eligible_vendor_ids=None)
    return _plan_response(result)


@router.post("/models/3/generate-plan-and-weekly-view", response_model=GeneratePlanAndWeeklyViewResponse)
def post_model3_generate_plan_and_weekly_view(body: GeneratePlanRequest):
    plan = generate_plan(body.available_funds, session=None, as_of=None, eligible_vendor_ids=None)
    weekly_view = build_weekly_view(
        plan["allocations"], session=None, as_of=None, model_used="model3", funds_figure=body.available_funds
    )
    return {
