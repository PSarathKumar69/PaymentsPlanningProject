...
New Model 2 (`14-new-model-2.md`) is built on two separate layers, so that every number Finance sees is always accurate, while still giving Finance a plain-language explanation behind each suggestion.
...
2. Finance enters the expected funds figure for the month — but only after New Model 2 has shown them the minimum funds required (see `14-new-model-2.md`). The system never asks for a funds figure with no context.
3. The system generates the full month's plan (New Model 2), split into weekly sub-plans (W1-W5) based on each vendor's Assigned Week.
4. Finance reviews the suggested plan and makes payments day by day; each payment is logged into the system manually (no file upload/reconciliation, no ERP integration, for this build — see `07-data-pipeline-and-master-sheet.md`).
